cube(`HybridKPIs`, {
  ...
  measures: {
    ...
    weeklyGrowth: {
      sql: `
        CASE 
          WHEN (SELECT SUM(solar_capacity+bess_storage_capacity+wind_capacity) 
          ...
          THEN ((SELECT SUM(solar_capacity+bess_storage_capacity+wind_capacity) 
                 FROM public.solar_chart_view 
                 WHERE commisioned_date >= CURRENT_DATE - INTERVAL '7 days') - 
                (SELECT SUM(solar_capacity+bess_storage_capacity+wind_capacity) 
                 FROM public.solar_chart_view 
                 WHERE commisioned_date >= CURRENT_DATE - INTERVAL '14 days' 

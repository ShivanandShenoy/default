cube(`EnergyInstallations`, {
  sql: `
    ...
    WHERE to_regclass('vw_energyinstallations_${COMPILE_CONTEXT.securityContext.tenant_id}') IS NOT NULL AND status_id IN (
...
// sql_table: `vw_energyinstallations_${COMPILE_CONTEXT.securityContext.tenant_id}`,
  ...
  measures: {
    ...
    totalCapacity: {
      sql: `COALESCE(${CUBE}.bess_storage_capacity, 0)`,
      type: `sum`,
  ...
  segments: {
    ...
    largeScale: {
      sql: `${CUBE}.bess_storage_capacity >= 100`,
    },

    mediumScale: {
      sql: `${CUBE}.bess_storage_capacity >= 10 AND ${CUBE}.bess_storage_capacity < 100`,
    },

    smallScale: {
      sql: `${CUBE}.bess_storage_capacity < 10`,
    },
